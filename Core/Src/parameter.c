...
/* ============ */
	...
	const stSPEED f_StSpeedData[PARAM_MOVE_SPEED_MAX] = {
		...
		{ 400.0,	7.5,			7.5,		0,		0,				0,				},		// 通常(PARAM_NORMAL)
		{ 500.0,	10.0,			10.0,		0,		0,				0,				},		// 高速(PARAM_FAST)
		{ 700.0,	15.0,			15.0,		0,		0,				0,				},		// 超高速(PARAM_VERY_FAST)
	};
	...
	const stSPEED f_SlaSpeedData[PARAM_MOVE_SPEED_MAX] = {
		...
		{ 0,	4.5,			4.5,		9000.0*PI,		10.0*PI,			10.0*PI,			},		// 通常(PARAM_NORMAL)
		{ 0,	4.5,			4.5,		22000.0*PI,		10.0*PI,			10.0*PI,			},		// 高速(PARAM_FAST)
...
/* ============== */
	...
	const stGAIN f_StGainData[PARAM_MOVE_SPEED_MAX] = {
		...
		/* 超低速(PARAM_VERY_SLOW) */
			...
			{ 30.0,4.0,0.1, 155.0,5.0,0.03,	0.0,0.0,	1.0,0.0,},
		/* 低速(PARAM_SLOW) */
			//速度kp,速度ki,速度kd,	角速度kp,角速度ki,角速度kd,	角度kp,角度ki,	壁kp,壁kd
			{ 30.0,4.0,0.1,	155.0,5.0,0.03,	0.0,0.0,	0.3,0.0,},
		/* 通常(PARAM_NORMAL) */
			//速度kp,速度ki,速度kd,	角速度kp,角速度ki,角速度kd,	角度kp,角度ki,	壁kp,壁kd
			{ 30.0,4.0,0.1,	155.0,5.0,0.03,	0.0,0.0,	0.3,0.0,},
		/* 高速(PARAM_FAST) */
		...
		/* 超高速(PARAM_VERY_FAST) */
			//速度kp,速度ki,速度kd,	角速度kp,角速度ki,角速度kd,	角度kp,角度ki,	壁kp,壁kd
			{ 30.0,4.0,0.1,	155.0,5.0,0.03,	0.0,0.0,	0.05,0.0,}
	};
	...
	const stGAIN f_TurnGainData[PARAM_MOVE_SPEED_MAX] = {
		...
		/* 超低速(PARAM_VERY_SLOW) */
			//速度kp,速度ki,速度kd,	角速度kp,角速度ki,角速度kd,	角度kp,角度ki,	壁kp,壁kd
			{ 40.0,4.0,0.1,	200.0,15.0,3.0,	0.0,0.0,	0.0,0.0,},
		/* 低速(PARAM_SLOW) */
			//速度kp,速度ki,速度kd,	角速度kp,角速度ki,角速度kd,	角度kp,角度ki,	壁kp,壁kd
			{ 40.0,4.0,0.1,	200.0,15.0,3.0,	0.0,0.0,	0.0,0.0,},
		/* 通常(PARAM_NORMAL) */
	...
	const stGAIN f_SlaGainData[PARAM_MOVE_SPEED_MAX] = {
		...
		/* 超低速(PARAM_VERY_SLOW) *///吸引なし　300
			//速度kp,速度ki,速度kd,	角速度kp,角速度ki,角速度kd,	角度kp,角度ki,	壁kp,壁kd
			{ 30.0,4.0,0.1,	240.0,8.0,0.5,	0.0,0.0,	1.0,0.0,},
