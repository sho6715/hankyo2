...
//			LL_TIM_EnableCounter(TIM4);
		...
		case MODE_3:
			...
			MOT_setSuraStaSpeed( SEARCH_SPEED );							// 繧ｹ繝ｩ繝ｭ繝ｼ繝?髢句ｧ矩溷ｺｦ險ｭ螳?
			PARAM_setSpeedType( PARAM_ST,   PARAM_VERY_SLOW );							// [逶ｴ騾ｲ] 騾溷ｺｦ譎ｮ騾?
			PARAM_setSpeedType( PARAM_TRUN, PARAM_VERY_SLOW );							// [譌句屓] 騾溷ｺｦ譎ｮ騾?
			PARAM_setSpeedType( PARAM_SLA,  PARAM_VERY_SLOW );							// [繧ｹ繝ｩ] 騾溷ｺｦ譎ｮ騾?
			SetLED(0x00);
			LL_mDelay(500);
			CTRL_clrNowData();
			CTRL_clrData();
...
void MODE_exe_m1( void )
{
	...
	switch( en_Mode ){
		...
		case MODE_5:
			SetLED(0x0e);
			MAP_setPos( 0, 0, NORTH );												// 繧ｹ繧ｿ繝ｼ繝井ｽ咲ｽｮ
			MAP_Goalsize(1);
			MAP_makeContourMap_run( GOAL_MAP_X_def, GOAL_MAP_Y_def, BEST_WAY );					// 遲蛾ｫ倡ｷ壹?槭ャ繝励ｒ菴懊ｋ
			MAP_makeCmdList( 0, 0, NORTH, GOAL_MAP_X_def, GOAL_MAP_Y_def, &en_endDir2 );		// 繝峨Λ繧､繝悶さ繝槭Φ繝我ｽ懈??
			MAP_makeSuraCmdList();													// 繧ｹ繝ｩ繝ｭ繝ｼ繝?繧ｳ繝槭Φ繝我ｽ懈??
			MAP_makeSkewCmdList();													
			MAP_showCmdLog();
			break;
...
bool MODE_setWaitCheck(void){
	...
	if( TRUE == MODE_DistRightCheck() ){	// 蜿ｳ縺?縺第､懃衍
		SetLED(0x08);
	}
	if( TRUE == MODE_DistLeftCheck() ){		// 蟾ｦ縺?縺第､懃衍
		SetLED(0x02);
	}
