...
//			LL_TIM_EnableCounter(TIM4);
		...
		case MODE_4:
			...
			MOT_setSuraStaSpeed( SEARCH_SPEED );							// 繧ｹ繝ｩ繝ｭ繝ｼ?ｿｽ?髢句ｧ矩溷ｺｦ險ｭ?ｿｽ?
			PARAM_setSpeedType( PARAM_ST,   PARAM_VERY_SLOW );							// [逶ｴ騾ｲ] 騾溷ｺｦ譎ｮ?ｿｽ?
			PARAM_setSpeedType( PARAM_TRUN, PARAM_VERY_SLOW );							// [譌句屓] 騾溷ｺｦ譎ｮ?ｿｽ?
			PARAM_setSpeedType( PARAM_SLA,  PARAM_VERY_SLOW );							// [繧ｹ繝ｩ] 騾溷ｺｦ譎ｮ?ｿｽ?
			SetLED(0x00);
			LL_mDelay(500);
			CTRL_clrNowData();
			CTRL_clrData();
...
bool MODE_setWaitCheck(void){
	...
	if( TRUE == MODE_DistRightCheck() ){	// 蜿ｳ?ｿｽ?縺第､懃衍
		SetLED(0x08);
	}
	if( TRUE == MODE_DistLeftCheck() ){		// 蟾ｦ?ｿｽ?縺第､懃衍
		SetLED(0x02);
	}
